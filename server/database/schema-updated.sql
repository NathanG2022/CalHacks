-- AI Safety Testing Webapp - Database Schema (Updated for Anonymous Users)
-- Execute this SQL in your Supabase SQL Editor

-- Test Runs: Each multi-model test session
CREATE TABLE IF NOT EXISTS test_runs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID, -- Nullable to allow anonymous users
...
DROP POLICY IF EXISTS "Users can insert test results" ON test_results;

-- Users can see their own test runs OR anonymous test runs (user_id IS NULL)
CREATE POLICY "Users can view own test runs" ON test_runs
  FOR SELECT USING (auth.uid() = user_id OR user_id IS NULL);

-- Users can insert their own test runs OR anonymous test runs
CREATE POLICY "Users can insert own test runs" ON test_runs
  FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL);

-- Users can update their own test runs OR anonymous test runs
...
CREATE POLICY "Users can view test results" ON test_results
  FOR SELECT USING (
    EXISTS (
      ...
      AND (test_runs.user_id = auth.uid() OR test_runs.user_id IS NULL)
    )
  );
